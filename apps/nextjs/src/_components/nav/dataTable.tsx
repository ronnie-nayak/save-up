"use client"
import { Button, Calendar, FormControl, Input, Popover, PopoverContent, PopoverTrigger, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Separator, cn } from "@acme/ui";
...
export function DataTable() {
  ...
  const endPage = Math.ceil(localData.length / 8)
  ...
  return (
    <div className="w-full text-left h-min bg-midnight m-10 p-6 rounded-3xl dark">
      <div className="flex gap-4 items-center">
        ...
        <Input type="text" placeholder="Search" className="w-1/3" onChange={(event) => router.replace(pathname + '?' + createQueryString('title', event.target.value))} />
      ...
      <header className="text-gray-500 flex m-9 my-4 text-xl">
        ...
        <section className="h-[550px]">
          {(localData.map((row, i) => (
            <div className="h-auto">
              <motion.div key={row.id} layoutId={"" + row.id} className="flex text-lg m-3"
              ...
              >
                ...
                <div className="w-3/12">
                  <div className="relative p-2 bg-bermuda rounded-full w-min flex gap-1 justify-center items-center text-base pl-8">
                    <button className={`absolute left-4 top-4 w-2 h-2 rounded-full animate-ping ${row.type === "income" ? "bg-green-500" : "bg-red-500"}`} />
                    <button className={`absolute left-4 top-4 w-2 h-2 rounded-full ${row.type === "income" ? "bg-green-500" : row.type === "expense" ? "bg-red-500" : "bg-orange-500"}`} />
                    {row.type}
                  </div>
                </div >
